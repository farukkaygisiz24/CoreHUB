...
const glassButtonVariants = cva(
  ...
  ),
  {
    variants: {
      variant: {
        default: cn(
          "bg-black/5 backdrop-blur-xl border border-black/10 text-slate-800",
          ...
          "dark:hover:bg-white/30 dark:hover:border-white/40",
        ...
        outline: cn(
          "bg-transparent backdrop-blur-sm border-2 border-black/15 text-slate-700",
          "hover:bg-black/5 hover:border-black/25",
          "dark:border-white/40 dark:text-white dark:hover:bg-white/10 dark:hover:border-white/60",
        ),
        ghost: cn(
    ...
    return (
      <div className="relative inline-block">
        ...
        <Comp className={cn(glassButtonVariants({ variant, size, className }))} ref={ref} {...props}>
          <span className="relative z-10 flex items-center gap-2 text-inherit">{children}</span>
        </Comp>
      </div>
    )
  },
