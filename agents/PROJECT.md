...
```
[RSS sources + Google Trends]  →  lib/ingest/run.ts  (shared core)
                     │
                     ├─ dedupeArticleStore() — merge duplicate cards already in store
                     ...
                     ↓
Entry points:
  • Local:  npm run ingest  →  scripts/ingest.ts  →  runIngest()
  • Prod:   vercel.json cron */10 * * * *  →  GET /api/cron/ingest  →  runIngest()
                     ↓
[Next.js App Router] loadArticles() → pages (ISR revalidate=300; revalidatePath after cron)
```

The website is **read-only** at request time; all generation happens in ingest.
Pages use ISR (`revalidate = 300`) and cron calls `revalidatePath` after each run.
...
| `lib/store.ts` | Persistence: Vercel Blob when `BLOB_READ_WRITE_TOKEN` set, else `data/articles.json`. |
| `scripts/ingest.ts` | Local CLI wrapper → `runIngest()`. Loads `.env.local` via dotenv. |
...
| `components/ThemeToggle.tsx` | Light/dark toggle via `useSyncExternalStore` + `GlassSwitch`; persists in `localStorage`. |
...
The rest of the system never knows which model is used. Configure an ordered chain:

```env
AI_CHAIN=groq:llama-3.3-70b-versatile,groq:llama-3.1-8b-instant,groq:meta-llama/llama-4-scout-17b-16e-instruct
```

`getAIProvider()` builds a `FallbackProvider`: uses the first model; on **daily** quota
(`QuotaExhaustedError` from Groq 429 + "per day") skips to the next. Per-minute 429s
retry with backoff inside `groq.ts`. Legacy `AI_PROVIDER` still works as a single entry.
...
Copy from `.env.example`. Local-only import templates (`vercel.env`, `vercel-import.env`) are gitignored.

| Var | Purpose |
|-----|---------|
| `AI_CHAIN` | Comma-separated provider:model chain (see §6). |
| `GROQ_API_KEY`, `GROQ_MODEL` | Groq credentials. |
| `GEMINI_API_KEY`, `GEMINI_MODEL` | Optional; not in default chain (TR free tier 0). |
| `UNSPLASH_ACCESS_KEY` | Unsplash fallback images. |
| `CRON_SECRET` | Protects `/api/cron/ingest` (Bearer token). |
| `BLOB_READ_WRITE_TOKEN` | Production article store (Vercel Blob). Omit locally → uses `data/articles.json`. |
| `INGEST_MAX_PER_RUN` | Max new articles per run (default 6). |
| `INGEST_MAX_AGE_DAYS` | RSS age window (default 3). |
| `INGEST_BOOTSTRAP_UNTIL` | While store count < this, backfill mode (default 50). |
| `INGEST_BOOTSTRAP_PER_FEED` | Items/feed in backfill (default 30). |
| `INGEST_BOOTSTRAP_MAX_PER_RUN` | Max articles/run in backfill (default 6; Pro can use 12). |

## 10. Running Locally

```bash
npm install
cp .env.example .env.local   # fill GROQ_API_KEY, UNSPLASH_ACCESS_KEY, AI_CHAIN
npm run ingest               # → data/articles.json (gitignored)
npm run dev                  # http://localhost:3000
```

**Note:** Without `BLOB_READ_WRITE_TOKEN`, ingest writes to `data/articles.json` only.
Production Blob is separate — local and live depots do not sync automatically.
To mirror live content locally, run ingest or copy from Vercel Blob manually.

## 11. Known Limitations / TODO

- **Storage:** Production uses **Vercel Blob** (`corehub/articles.json`, private store).
  Local dev uses gitignored `data/articles.json`. No DB yet.
- **Cron:** Wired — `vercel.json` `*/10 * * * *` → `/api/cron/ingest` (Pro plan).
- **Store pruning:** Old articles are **not** auto-deleted; only duplicate **merging**
  runs each ingest. Optional future: `STORE_MAX_AGE_DAYS`.
- **Old records:** Changing `Article` shape requires re-ingesting or clearing the store.
...
- **2026-07-01** — **Cron auth fallback.** `/api/cron/ingest` accepts Vercel cron
  ...
  `hasBlobStorage` in `lib/store.ts`.
- **2026-07-01** — **3-model AI chain (no Gemini).** Default production chain:
  `groq:llama-3.3-70b-versatile` → `groq:llama-3.1-8b-instant` →
  `groq:meta-llama/llama-4-scout-17b-16e-instruct` (wider context fallback for 413).
